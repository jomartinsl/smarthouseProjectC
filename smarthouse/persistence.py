    ...
    def load_smarthouse_deep(self):
        ...
        for dev in result.get_devices():
            if isinstance(dev, Actuator):
                cursor.execute(f"SELECT state FROM actuator_state where device_id = '{dev.id}';")
...
if __name__ == "__main__":
    ...
    print(h)
    repo.calc_avg_temperatures_in_room(h.get_rooms()[0], "2024-01-27", "2024-01-28")
    repo.calc_hours_with_humidity_above(h.get_rooms()[0], "2024-01-27")
